...
async function convertSelectedFiles() {
  ...
  if (status) status.textContent = t('hero.statusConvertingCount', { count: state.selectedFiles.length });
  showToast(t('warnings.converting', { count: state.selectedFiles.length }), 'success', 1400);

  for (const file of state.selectedFiles.slice()) {
    try {
      if (status) status.textContent = t('hero.statusConvertingFile', { name: file.name });
      await convertSingleFile(file, mode);
      if (status) status.textContent = t('hero.statusConvertedFile', { name: file.name });
  ...
  if (status) status.textContent = t('hero.statusComplete');
  showToast(t('warnings.converted'), 'success', 1800);
  setTimeout(() => {
    if (status && status.textContent === t('hero.statusComplete')) status.textContent = '';
  }, 4000);
