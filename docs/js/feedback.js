...
export function initFeedback(root, options) {
  if (!root) return;
  ...
  const listWrapper = root.querySelector('.feedback-list-wrapper');
  ...
  async function fetchFeedback() {
    ...
    try {
      const response = await fetch(`${config.endpoint}?t=${Date.now()}`, { cache: 'no-store' });
      ...
      const sorted = normalized.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0) || String(a.id).localeCompare(String(b.id)));
      state.items = sorted;
      renderComments();
    } catch (error) {
      console.error(error);
  ...
  function wireEvents() {
    ...
    window.addEventListener('resize', () => {
      ...
      state.resizeTimer = setTimeout(updateLayout, 120);
    }, { passive: true });
  }
  ...
  function boot() {
    ...
    wireEvents();
    fetchFeedback();
    state.refreshTimer = setInterval(fetchFeedback, 30000);
